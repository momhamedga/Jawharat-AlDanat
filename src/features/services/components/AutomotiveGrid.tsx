...
interface AutomotiveGridProps {
  locale: string;
}

export default function AutomotiveGrid({ locale }: AutomotiveGridProps) {
  const isAr = locale === 'ar';
  const automotiveServices = getServicesByCategory('automotive');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-16">
        ...
        <div className="max-w-3xl space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/20 bg-primary/5 text-primary text-xs font-semibold">
            ...
            <span>{isAr ? 'القسم الأول • العناية المتقدمة بالسيارات' : 'Division 01 • Advanced Automotive Care'}</span>
          </div>

          <h2 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight leading-tight">
            {isAr
              ? 'حلول هندسية متطورة لحماية وتجميل أسطول السيارات الفارهة'
              : 'Engineered Solutions for Luxury Fleet Protection & Restoration'}
          </h2>

          <p className="text-base text-muted-foreground leading-relaxed">
            {isAr
              ? 'تعتمد ورش عملنا في أبوظبي ودبي على أحدث مواد البولي يوريثان والنانو سيراميك وتقنيات البخار الجاف، لضمان أعلى درجات الحماية والقيمة الاستثمارية لمركبتك.'
              : 'Our state-of-the-art facilities in Abu Dhabi and Dubai leverage high-grade TPU films, certified nano ceramics, and dry vapor detailing to preserve maximum vehicle value.'}
          </p>
        </div>

        {/* 🧱 4 Tailored Panels Grid */}
            ...
            >
              ...
              <div className="relative w-full aspect-[16/10] overflow-hidden bg-muted">
                ...
                <div className="absolute top-4 start-4 px-3 py-1 rounded-full bg-background/90 backdrop-blur-md border border-border/80 text-[11px] font-bold text-primary">
                  {isAr ? `خدمة 0${idx + 1}` : `Service 0${idx + 1}`}
                </div>
              </div>
