...
export default async function VipEventManagementPage({ params }: Props) {
  ...
  // 📊 فصل وهيكلة البيانات لـ 6 ركائز الفاخرة بنقاء لغوي مطلق لمنع تداخل الـ Hydration
  const vipPillarsData = [
    {
      id: 'royal-diwan',
      ar: { title: 'تخطيط المجالس الملكية', subTitle: 'Royal Diwan Planning' },
      en: { title: 'Royal Diwan Planning', subTitle: 'تخطيط المجالس الملكية' },
      icon: (
    ...
    {
      id: 'vip-protocol',
      ar: { title: 'بروتوكول واستقبال الاستضافة', subTitle: 'VIP Protocol & Seating' },
      en: { title: 'VIP Protocol & Seating', subTitle: 'بروتوكول واستقبال الاستضافة' },
      icon: (
        <svg className="w-6 h-6 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    {
      id: 'high-end-av',
      ar: { title: 'الأنظمة السمعية والبصرية', subTitle: 'High-End AV Systems' },
      en: { title: 'High-End AV Systems', subTitle: 'الأنظمة السمعية والبصرية' },
      icon: (
        <svg className="w-6 h-6 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      ar: { title: 'تغطية وتصوير سينمائي', subTitle: 'Cinematic Coverage' },
      en: { title: 'Cinematic Coverage', subTitle: 'تغطية وتصوير سينمائي' },
      icon: (
        <svg className="w-6 h-6 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    {
      id: 'guest-relations',
      ar: { title: 'إدارة وتنسيق القوائم', subTitle: 'Elite Guest Relations' },
      en: { title: 'Elite Guest Relations', subTitle: 'إدارة وتنسيق القوائم' },
      icon: (
        <svg className="w-6 h-6 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    {
      id: 'post-gala',
      ar: { title: 'إشراف ما بعد الحفل', subTitle: 'Post-Gala Supervision' },
      en: { title: 'Post-Gala Supervision', subTitle: 'إشراف ما بعد الحفل' },
      icon: (
        <svg className="w-6 h-6 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
  ...
  // ⚡ المعالجة المباشرة والسريعة للمصفوفة بناءً على الـ Locale الحالي
  const vipPillars = vipPillarsData.map(pillar => ({
    id: pillar.id,
    icon: pillar.icon,
  ...
  return (
    <main className="w-full min-h-screen bg-[#F8FAFC] font-['Cairo'] select-none text-slate-800" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* 👑 هيدر ممتد وواسع مخصص لتفادي تداخل العناصر وبث هوية الـ VIP الفاخرة */}
      ...
      <section className="w-full py-12 md:py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 bg-white rounded-[32px] border border-slate-200/80 shadow-[0_20px_50px_rgba(3,15,30,0.02)] overflow-hidden items-center p-6 sm:p-10 lg:p-12">
          ...
          {/* الجانب الموجه للنصوص والـ 6 ركائز الأساسية - تم تصحيح المحاذاة لـ text-start للتوافق اللغوي التام */}
          <div className="col-span-1 lg:col-span-7 space-y-8 order-2 lg:order-1">
            <div className="space-y-3 text-start">
              <h2 className="text-2xl sm:text-3xl font-black text-brand-bg tracking-tight flex items-center gap-2">
          ...
          {/* الجانب المخصص للصورة والكارت الإحصائي المعلق - تم إصلاح كلاس التموضع منطقياً للـ RTL والـ LTR */}
          <div className="col-span-1 lg:col-span-5 relative w-full aspect-[4/3] sm:aspect-[16/11] lg:h-[550px] rounded-2xl overflow-hidden border border-slate-200 shadow-sm order-1 lg:order-2 group">
            ...
            {/* 🎯 البطاقة العائمة العلوية - تم تحويلها للحواف المنطقية لعدم الخروج عن إطار الشاشة */}
            <div className="absolute top-4 rtl:left-4 ltr:right-4 bg-brand-bg border-2 border-brand-deep-gold text-white rounded-xl py-3 px-5 text-center shadow-md z-20 transition-all duration-300 hover:scale-105">
      ...
      {/* 🔗 توجيه سفلي ذكي يحترم قواعد الـ Bidirectional Navigation للأسهم */}
