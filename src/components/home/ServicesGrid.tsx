  ...
  const services = [
    {
      ...
      title: isAr ? 'التلميع الاحترافي وطبقات السيراميك' : 'Polishing & Ceramic Coating',
      desc: isAr ? 'حماية ولمعان فائق لطلاء السيارة' : 'Ultimate shine & protection for your vehicle'
    },
    {
      ...
      title: isAr ? 'أفلام حماية الطلاء PPF' : 'Paint Protection Film (PPF)',
      desc: isAr ? 'درع شفاف لحماية السيارة من الخدوش والعوامل الجوية' : 'An invisible shield protecting against scratches & chips'
    },
    {
      ...
      title: isAr ? 'إدارة الفعاليات والمؤتمرات' : 'Events & Conferences Management',
      desc: isAr ? 'تنظيم متكامل للمؤتمرات والمعارض باحترافية عالية' : 'Comprehensive planning for corporate corporate gatherings'
    },
    {
      ...
      title: isAr ? 'تنظيم مناسبات كبار الشخصيات VIP' : 'VIP Event Management',
      desc: isAr ? 'تخطيط وإدارة الفعاليات النخبوية والبروتوكولات الرسمية' : 'Exclusive execution for high-profile elite suites'
    },
    {
      ...
      title: isAr ? 'التنظيف العميق بالبخار والـ Detailing' : 'Deep Cleaning & Detailing',
      desc: isAr ? 'تطهير شامل وعناية دقيقة بداخلية وخارجية السيارة' : 'Meticulous interior & exterior deep sanitization'
    },
    {
      ...
      title: isAr ? 'أفلام العزل الحراري والتظليل النانو' : 'Nano Ceramic Heat Insulation Film',
      desc: isAr ? 'حجب مثالي للأشعة الضارة وتقليل درجات الحرارة للمقصورة' : 'Premium thermal blocks safeguarding your cabin comfort'
    }
  ];
  ...
  return (
    <section className="relative w-full bg-slate-50 py-16 lg:py-24 overflow-hidden font-['Cairo'] select-none" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 xl:px-16 relative z-10">
        ...
        {/* 👑 هيدر القسم - منقسم بالكامل حسب اللغة */}
        <div className="w-full flex flex-col md:flex-row items-center justify-between border-b border-brand-deep-teal/10 pb-6 mb-12 gap-4">
          <div className={isAr ? 'text-center md:text-start w-full md:w-auto' : 'text-center md:text-left w-full md:w-auto'}>
            <h2 className="text-3xl sm:text-4xl font-black text-brand-bg tracking-tight">
              {isAr ? 'شبكة الخدمات الحصرية' : 'Our Premium Services'}
            </h2>
            <p className="text-xs md:text-sm font-bold text-brand-turquoise mt-2 tracking-wide uppercase">
              {isAr ? 'خدماتنا الأساسية — مصممة لتجربة مستخدم استثنائية' : 'Exclusive Pillars — Engineered for absolute perfection'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 items-stretch">
          {services.map((service) => (
            ...
            >
              ...
              {/* العنوان الصافي الخالي من التداخل اللغوي */}
              ...
              {/* وصف الخدمة باللغة النشطة لملء الفراغ الهيكلي بشكل جذاب */}
              <p className="mt-2 text-xs font-bold text-slate-400 tracking-wide line-clamp-2 px-2">
                {service.desc}
              </p>
