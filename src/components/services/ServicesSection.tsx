...
export default function ServicesSection({ locale }: ServicesSectionProps) {
  ...
  const services = [
    {
      ...
      title: isAr ? 'تلميع السيارات وبولیشنج السيراميك' : 'Car Polishing & Ceramic Coating',
      icon: (
    ...
    },
    {
      ...
      title: isAr ? 'حماية الطلاء والأفلام العازلة' : 'Paint Protection & PPf Films',
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      title: isAr ? 'تنظيم الفعالیات والمؤتمرات الكبرى' : 'Events & Major Conferences Management',
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      title: isAr ? 'إدارة وتنسيق فعاليات كبار الشخصيات' : 'VIP Event Management & Coordination',
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      title: isAr ? 'خدمات التنظيف العمیق والعناية الشاملة' : 'Deep Cleaning & Auto Detailing Services',
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      title: isAr ? 'العزل الحراري المتطور وحماية الزجاج' : 'Advanced Heat Insulation & Window Protection',
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 text-brand-deep-gold" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
  ...
  return (
    <section className="relative w-full py-16 md:py-28 bg-slate-50 overflow-hidden font-['Cairo'] select-none" dir={isAr ? 'rtl' : 'ltr'}>
      
      {/* 🌌 تأثير الإضاءة المحيطية مع المصفوفة المنقطة */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* 👑 هيدر القسم مع التبديل اللغوي الصافي */}
        <div className="text-center space-y-3 mb-12 md:mb-24 flex flex-col items-center">
          <span className="text-[10px] md:text-xs font-black tracking-[0.2em] text-brand-deep-gold bg-brand-light-gold/15 px-4 py-1.5 rounded-full uppercase">
            {isAr ? 'مجموعة الخدمات الرئيسية' : 'Core Solutions Portfolio'}
          </span>
          <h2 className={`text-2xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tight ${!isAr && 'font-sans'}`}>
            {isAr ? 'شبكة الخدمات الاحترافية' : 'Professional Services Grid'}
          </h2>
          ...
          <p className={`text-xs sm:text-sm font-semibold text-slate-500 max-w-xl leading-relaxed ${!isAr && 'font-sans'}`}>
            {isAr 
              ? 'خدماتنا الأساسية مصممة بعناية فائقة لتلبية تطلعاتكم وتقديم تجربة مستخدم استثنائية متكاملة' 
              : 'Our core services are meticulously engineered to exceed expectations and deliver an exceptional, seamless experience.'}
          </p>
        </div>

        {/* 🎴 شبكة الكروت المتجاوبة الذكية */}
          ...
          {services.map((service) => (
            ...
            >
              ...
              <div className="space-y-2 w-full flex-grow flex items-center justify-center">
                <h3 className={`text-lg md:text-xl font-black text-slate-800 tracking-tight leading-snug transition-colors duration-300 md:group-hover:text-brand-deep-gold ${!isAr && 'font-sans'}`}>
                  {service.title}
                </h3>
              </div>

              {/* رابط التوجيه التفاعلي مع قلب السهم والاتجاه تلقائياً */}
              <div className="mt-5 flex items-center gap-1.5 text-xs font-black text-brand-deep-gold transition-all duration-500
                ...
                <span className={!isAr ? "font-sans" : undefined}>
                  {isAr ? 'اكتشف تفاصيل الخدمة' : 'Discover Service Details'}
                </span>
                
                <svg 
                  className={`w-3.5 h-3.5 transition-transform duration-300 ${isAr ? '' : 'rotate-180'}`} 
                  fill="none" 
                  stroke="currentColor" 
                  strokeWidth="2.5" 
                  viewBox="0 0 24 24"
